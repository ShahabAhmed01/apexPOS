...
| `modifiers` / `modifier_groups` / `modifier_options` | Modifier system |
| `tax_rates` | Code, name, rate_bps, is_default |
...
| `table_sessions` | Open table sessions (guests, opened_at) |
...
- `idx_shifts_register_status` (register_id, status)
...
- Current: `SCHEMA_0001` (full schema)
...
- Creates: 1 org, 1 branch, 1 register, 10 categories, 82 products (12 variants), 10 users/roles, 10 tables, 12 customers, 2 gift cards, 3 suppliers, 25 menu items, 18% tax
