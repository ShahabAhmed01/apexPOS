...
export const Select = forwardRef<HTMLSelectElement, Props>(
  ({ label, options, onChange, className = '', ...rest }, ref) => (
    <label className="block">
      {label && <span className="mb-1.5 block text-xs font-medium text-[var(--color-text-1)]">{label}</span>}
      <span className="relative block">
        ...
        <ChevronDown size={14} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--color-text-2)]" aria-hidden />
      </span>
    </label>
  )
)
