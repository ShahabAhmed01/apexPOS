...
/**
 * Idempotent first-run seed. Safe to call on every boot — it only inserts
 * when the database is empty (no organizations). Demo credentials are
 * documented in README; production deployments should rotate them.
 */
export const seedIfEmpty = (db: DB): boolean => {
  ...
  const tx = db.transaction(() => {
    ...
    const rolePerms: Record<string, string[]> = {
      ...
      Manager: [
        'sales.view', 'sales.create', 'sales.void', 'sales.refund', 'sales.exchange',
        'discounts.apply', 'discounts.override', 'payments.take', 'payments.refund',
        'inventory.view', 'inventory.manage', 'inventory.adjust', 'inventory.receive',
        ...
        'tables.view', 'tables.manage', 'tables.transfer', 'kitchen.view', 'kitchen.manage',
        'reports.view', 'reports.export',
        'register.open', 'register.close', 'cash.adjust', 'cash.no_sale', 'cash.view_variance',
        'users.view', 'audit.view', 'data.backup'
      ],
      Cashier: [
        'sales.view', 'sales.create', 'discounts.apply', 'payments.take',
        'customers.view', 'tables.view', 'register.open', 'register.close'
      ],
      Waiter: ['sales.view', 'sales.create', 'tables.view', 'kitchen.view'],
      'Kitchen Staff': ['kitchen.view', 'kitchen.manage'],
      'Inventory Manager': [
        'inventory.view', 'inventory.manage', 'inventory.adjust', 'inventory.receive',
        'inventory.transfer', 'inventory.count', 'purchases.view', 'reports.view'
      ],
      'Purchasing Manager': [
        'purchases.view', 'purchases.create', 'purchases.approve', 'purchases.receive',
    ...
    for (const [username, display, password, role] of userSeed) {
      userInsert.run(
        ...
        hashPassword('1234'),
        roleIds[role]!, branchId, t
      )
    }

    // ---- Units --------------------------------------------------------------
    const unitInsert = db.prepare(
      'INSERT INTO units (id, code, name, decimals) VALUES (?, ?, ?, ?)'
    )
    const units: [string, string, number][] = [
      ['pc', 'Piece', 0], ['box', 'Box', 0], ['carton', 'Carton', 0],
      ['kg', 'Kilogram', 3], ['g', 'Gram', 0], ['L', 'Litre', 3], ['ml', 'Millilitre', 0]
    ]
    ...
    const cats = [
      'Beverages', 'Snacks & Confectionery', 'Grocery Staples', 'Dairy & Eggs',
      'Personal Care', 'Household', 'Bakery', 'Frozen Foods'
    ]
    cats.forEach((c, i) => {
      const cid = id()
      catIds[c] = cid
    ...
    const suppliers: [string, string, string, string, string][] = [
      ['Metro Distribution Co.', 'Asif Raza', '0301-1234567', 'orders@metrod.pk', 'Industrial Area, Karachi'],
      ['Haleeb Foods Supply', 'Sana Tariq', '0302-7654321', 'supply@haleeb.pk', 'Lahore Road, Sheikhupura'],
...
// ---------------------------------------------------------------------------
const P = (
  sku: string, barcode: string, name: string, cat: string, price: number,
  cost: number, unit: string, opts: Partial<{ weighted: boolean; low: number }> = {}
...
function seedProducts(
  ...
  const products = [
    ...
    P('BEV-007', '8961001000073', 'Lipton Yellow Label 100 Tea Bags', 'Beverages', 78000, 68000, 'pc'),
    ...
    P('SNK-002', '8961001100026', 'Kurkure Red Chutney 45g', 'Snacks & Confectionery', 6000, 4500, 'pc'),
    P('SNK-003', '8961001100033', 'Super Crisp BBQ 48g', 'Snacks & Confectionery', 5500, 4200, 'pc'),
    P('SNK-004', '8961001100040', 'Prince Biscuit Roll Pack', 'Snacks & Confectionery', 8000, 6000, 'pc'),
    P('SNK-005', '8961001100057', 'Oreo Original 144g', 'Snacks & Confectionery', 28000, 22000, 'pc'),
    P('SNK-006', '8961001100064', 'CandyLand Chilli Milli 36g', 'Snacks & Confectionery', 4000, 2800, 'pc'),
    P('SNK-007', '8961001100071', 'KitKat 4 Finger 41.5g', 'Snacks & Confectionery', 25000, 20000, 'pc'),
    P('SNK-008', '8961001100088', 'Cadbury Dairy Milk 38g', 'Snacks & Confectionery', 22000, 17500, 'pc'),
    P('SNK-009', '8961001100095', 'Bisconni Chocolate Chip 8-pack', 'Snacks & Confectionery', 18000, 14000, 'pc'),
    P('SNK-010', '8961001100101', 'Peek Freans Gluco 6-roll', 'Snacks & Confectionery', 20000, 15500, 'pc'),
    // Grocery Staples
    P('GRC-001', '8961001200016', 'Basmati Rice Premium (loose)', 'Grocery Staples', 42000, 35000, 'kg', { weighted: true, low: 10000 }),
    P('GRC-002', '8961001200023', 'Atta Whole Wheat 10kg Bag', 'Grocery Staples', 175000, 155000, 'pc'),
    P('GRC-003', '8961001200030', 'Dal Chana (loose)', 'Grocery Staples', 38000, 31000, 'kg', { weighted: true }),
    P('GRC-004', '8961001200047', 'Sugar (loose)', 'Grocery Staples', 16000, 14000, 'kg', { weighted: true }),
    P('GRC-005', '8961001200054', 'Salt Iodized 800g', 'Grocery Staples', 8000, 6000, 'pc'),
    P('GRC-006', '8961001200061', 'Dal Mong (loose)', 'Grocery Staples', 46000, 38000, 'kg', { weighted: true }),
    ...
    // Dairy
    P('DRY-001', '8961001300013', 'Milk Fresh 1L Tetra', 'Dairy & Eggs', 32000, 27000, 'pc', { low: 12000 }),
    ...
    P('DRY-007', '8961001300075', 'Tea Whitener Sachet Box 24pc', 'Dairy & Eggs', 48000, 40000, 'box'),
    // Personal Care
    P('PRC-001', '8961001400010', 'Lifebuoy Soap 115g', 'Personal Care', 15000, 12000, 'pc'),
    P('PRC-002', '8961001400027', 'Lux Soap Silk Sensation 110g', 'Personal Care', 17000, 13500, 'pc'),
    ...
    P('FRZ-003', '8961001700035', 'Mixed Vegetables 1kg Frozen', 'Frozen Foods', 42000, 34000, 'pc'),
  ...
  const txy = db.transaction(() => {
    ...
    insertP.run(teeId, 'APR-001', null, 'Cotton T-Shirt (Crew Neck)',
      catIds['Grocery Staples'] ?? null, unitIds['pc']!, 150000, 95000, taxStd, 0, null, t, t)
    ...
    for (const size of ['S', 'M', 'L', 'XL']) {
      for (const color of ['Black', 'White', 'Navy']) {
        ...
        vIns.run(vId, teeId, `APR-001-${color.slice(0, 1)}${size}`,
          `89610018${String(Math.floor(Math.random() * 1e6)).padStart(6, '0')}${size}`,
          `${color} / ${size}`,
          JSON.stringify({ color, size }), 150000, 95000)
...
function seedRestaurant(db: DB, branchId: string): void {
  ...
  const positions: [number, number][] = [
    [40, 40], [180, 40], [320, 40], [460, 40], [40, 180], [180, 180], [320, 180], [460, 180]
  ]
  const tx = db.transaction(() => {
    positions.forEach(([x, y], i) => {
      const isRound = i % 3 === 0
      tIns.run(crypto.randomUUID(), mainId, `T-${i + 1}`, isRound ? 2 : 4, isRound ? 'round' : 'square', x, y, 90, 90)
    })
    ...
    for (const [name, delta, def] of [['Mild', 0, 0], ['Medium', 0, 1], ['Hot', 0, 0], ['Extra Hot', 0, 0]] as const) {
      oIns.run(crypto.randomUUID(), spice, name, delta, def)
    }
    const extras = crypto.randomUUID()
    gIns.run(extras, 'Extras', 0, 4, 0)
    for (const [name, delta] of [['Extra Cheese', 8000], ['Garlic Bread', 6000], ['Side Salad', 5000], ['Raita', 3000]] as const) {
    ...
    const pmg = db.prepare('INSERT INTO product_modifier_groups (product_id, group_id) VALUES (?, ?)')
