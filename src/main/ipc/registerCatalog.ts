...
export const registerCatalogIpc = (services: Services, sessionStore: SessionStore): void => {
  ...
  handle(IpcChannel.ProductsByBarcode, {
  ...
  handle(IpcChannel.RegisterOpen, {
    permission: 'register.open',
    schema: z.object({ registerId: z.string().uuid(), openingFloat: z.number().int().nonnegative() }),
  ...
  handle(IpcChannel.RegisterCurrent, {
    handler: () => {
      const rows = db
        .prepare(`SELECT * FROM shifts WHERE status = 'open' AND branch_id = (SELECT id FROM branches LIMIT 1)`)
        .all() as { id: string; register_id: string }[]
      if (rows.length === 0) return null
      return registers.byId(rows[0]!.id)
    }
  }, services, () => sessionStore.get())

  handle(IpcChannel.RegisterClose, {
    permission: 'register.close',
    schema: z.object({ countedCash: z.number().int().nonnegative(), note: z.string().max(240).optional(), blind: z.boolean().optional() }),
    handler: (_ctx, input: { countedCash: number; note?: string; blind?: boolean }) => {
      ...
      return registers.close(open.register_id, input.countedCash, sessionOr(sessionStore).userId, input.note)
    }
  }, services, () => sessionStore.get())

  handle(IpcChannel.RegisterCashMovement, {
    ...
    handler: (_ctx, input: { kind: 'pay_in' | 'pay_out'; amount: number; reason: string }) => {
      ...
      if (input.kind === 'pay_in') registers.payIn(open.id, input.amount, input.reason, sessionOr(sessionStore).userId)
