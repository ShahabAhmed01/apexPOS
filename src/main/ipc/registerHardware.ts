...
export const registerHardwareIpc = (services: Services, sessionStore: SessionStore): void => {
  ...
  handle(
    IpcChannel.HardwareTest,
    {
      requiresAuth: true,
      schema: z.object({ device: z.string().min(1).max(32) }),
  ...
  handle(
    IpcChannel.HardwarePrintReceipt,
    {
  ...
  handle(
    IpcChannel.HardwareCustomerDisplay,
    {
      requiresAuth: true,
