...
export const registerCustomersIpc = (services: Services, sessionStore: SessionStore): void => {
  ...
  handle(
    IpcChannel.CustomersAdjustLoyalty,
    {
  ...
  handle(
    IpcChannel.GiftCardsIssue,
    {
      ...
      handler: (_ctx, input: { code: string; amount: number }) =>
        customers.issueGiftCard(input.code, input.amount)
    },
    services,
    () => sessionStore.get()
  )
